...
void llama_model_deepseek4::load_arch_hparams(llama_model_loader & ml) {
...

    {
        ...
        // a layer reads the caches of the last source at or before it; with V4's rule every
        // layer that compresses is its own source, so this is the identity there
        int32_t src = -1;
        for (uint32_t il = 0; il < hparams.n_layer_all; ++il) {
...
// dsv4_comp_pool: the aggregator follows the model, overlapped or disjoint.
// the compression ratios come from the model: V4 uses 4 and 128, V4.1 uses 2 and 1.
// They are read from the cache context, which already derives them from hparams.
...
// The gate of a ratio-1 compressor has no weights: with one element per group the softmax
// is 1.0 whatever the score, so the pooling collapses to norm(wkv(x)) -- which is what the
// reference returns for ratio 1. Zeros keep the existing path exact instead of forking it.
static ggml_tensor * dsv4_comp_score(ggml_context * ctx0, ggml_tensor * wgate, ggml_tensor * kv,
        ggml_tensor * cur, int64_t ratio, const llama_model_deepseek4::graph * g) {
    if (wgate) {
        return g->build_lora_mm(wgate, cur);
    }
    GGML_ASSERT(ratio == 1 && "DSV4: compressor without a gate must have ratio 1");
    return ggml_scale(ctx0, kv, 0.0f);
}
...
ggml_tensor * llama_model_deepseek4::graph::build_hca_compressed_kv_from_state(
    ...
    // the index keys are built from this, before the rope
    if (pre_rope) {
        *pre_rope = comp;
    }
...
ggml_tensor * llama_model_deepseek4::graph::build_v41_index_key(
    ...
    ggml_tensor * k = build_lora_mm(layer.indexer_attn_k, latent);
    k = build_norm(k, layer.indexer_k_norm, nullptr, LLM_NORM_RMS, il);
    ...
    // a latent stands for the first token of its group, and comp_pos already holds that position
    k_pe = ggml_rope_ext(ctx0, k_pe, comp_pos, nullptr, n_embd_ik_rope, rope_type, n_ctx_orig,
            hparams.dsv4_compress_rope_base, freq_scale, ext_factor,
            dsv4_rope_attn_factor(freq_scale, ext_factor), beta_fast, beta_slow);
...
ggml_tensor * llama_model_deepseek4::graph::build_lid_top_k(
    ...
    // V4.1 shares the index keys from the owning layer; in V4 the source is the layer itself
    const int32_t lid_src_il = hparams.dsv4_kv_src_layer[il];
    GGML_ASSERT(lid_src_il >= 0 && "DSV4: layer reads index keys with no source");
    ggml_tensor * indexer_k = inp_dsv4->mctx->get_lid()->get_k(ctx0, lid_src_il);
    const int64_t n_lid = score_mask->ne[0];
    GGML_ASSERT(n_lid > 0);
    ...
    if (cparams.fused_lid) {
        indexer_score = ggml_lightning_indexer(ctx0, indexer_q, indexer_k, indexer_weights, score_mask);
        ...
        indexer_score = ggml_add(ctx0, indexer_score, score_mask);
        cb(indexer_score, "lid_score_masked", il);
    }
...
ggml_tensor * llama_model_deepseek4::graph::build_csa_lid_attention(
    ...
    const int32_t csa_src_il = hparams.dsv4_kv_src_layer[il];
    GGML_ASSERT(csa_src_il >= 0 && "DSV4: layer reads compressed KV with no source");
    ggml_tensor * csa_k = use_hca
        ? inp_dsv4->mctx->get_hca()->get_k(ctx0, csa_src_il)
        : inp_dsv4->mctx->get_csa()->get_k(ctx0, csa_src_il);
...
ggml_tensor * llama_model_deepseek4::graph::build_hca_attention(
    ...
    GGML_ASSERT(hca_src_il >= 0 && "DSV4: layer reads compressed KV with no source");
    ggml_tensor * hca_k = inp_dsv4->mctx->get_hca()->get_k(ctx0, hca_src_il);
...
ggml_tensor * llama_model_deepseek4::graph::build_attention_impl(
    ...
    // V4 gets 4 and 128 here, the same values the constants used to hold
    const int64_t csa_ratio = inp_dsv4 ? (int64_t) inp_dsv4->mctx->get_csa_ratio() : 0;
    const int64_t hca_ratio = inp_dsv4 ? (int64_t) inp_dsv4->mctx->get_hca_ratio() : 0;
    GGML_UNUSED(csa_ratio);
    GGML_UNUSED(hca_ratio);
    ...
    if (ratio == hca_ratio && hparams.dsv4_is_kv_source[il] && inp_dsv4->get_hca().state_pos) {
        ...
        hca_state_score = dsv4_comp_score(ctx0, layer.attn_comp_wgate, hca_state_kv, cur, ratio, this);
        cb(hca_state_score, "hca_state_score", il);

        // V4.1 has no absolute position embedding on the gate
    ...
    if (ratio == csa_ratio && hparams.dsv4_is_kv_source[il] && inp_dsv4->get_csa().state_pos) {
        ggml_tensor * csa_state_kv = build_lora_mm(layer.attn_comp_wkv, cur);
        cb(csa_state_kv, "csa_state_kv", il);

        ggml_tensor * csa_state_score = dsv4_comp_score(ctx0, layer.attn_comp_wgate, csa_state_kv, cur, ratio, this);
        ...
        const bool overlap = inp_dsv4->mctx->get_comp_overlap();
        ggml_tensor * kv_comp_csa_state = overlap
            ? build_overlap_compressed_kv_from_state(
                ...
                "csa_state_compress",
                il, &csa_latent)
            : build_hca_compressed_kv_from_state(
                csa_source_kv,
                csa_source_score,
                inp_dsv4->get_csa().state_read_idxs,
                inp_dsv4->get_csa().state_write_pos,
        ...
        // V4 only: a second compressor in indexer space. V4.1 has none of these tensors.
        ...
        const bool lid_overlap = inp_dsv4->mctx->get_comp_overlap();
        ggml_tensor * kv_comp_lid_state = lid_overlap
            ? build_overlap_compressed_kv_from_state(
                lid_source_kv,
                lid_source_score,
    ...
    if (ratio == hca_ratio && hparams.dsv4_is_kv_source[il] && inp_dsv4->get_hca().state_write_idxs) {
        ...
        // layer 20 of V4.1 is both the ratio-1 kv source and an index source
        if (layer.indexer_attn_k) {
